      ...
      <div class="video-container">
        <div v-for="video in videos" :key="video.id" class="video-item" @click="mostrarModal(video)">
          ...
          <div class="video-thumbnail-container">
            <img :src="video.thumbnailUrl" alt="Miniatura del video" class="video-thumbnail" />
...
export default {
  ...
  setup() {
    ...
    const obtenerVideosDesdeServidor = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Error al obtener los videos desde el servidor:', response.statusText);
        }
      } catch (error) {
  ...
  onMounted(() => {
    ...
    const eliminarVideoEnBaseDeDatos = async (videoId) => {
      try {
        const response = await fetch(`http://18.218.183.17:3000/api/videos/${videoId}`, {
          method: 'DELETE',
        });
        if (response.ok) {
